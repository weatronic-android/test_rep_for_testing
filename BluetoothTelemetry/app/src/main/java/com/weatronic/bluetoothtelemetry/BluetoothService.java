...
public class BluetoothService{
        ...
        progress.setMessage(progMsgBase + "\n" + ctx.getString(R.string.Next_attempt_in) + " " + timeOutSec + " " + ctx.getString(R.string.Seconds) + "...");
            ...
            new CountDownTimer(timeOutSec * 1000, 1000){
                public void onFinish(){
                    //check if cancelled
                    if(reconnecting) {
                        progress.setMessage(ctx.getString(R.string.Attempt) + " " + retriesCur + " " + ctx.getString(R.string.Out_of) + " " + retriesMax + "\n"+ctx.getString(R.string.Attempting_reconnect));
                        connect(prefs.getString("lastSuccessfulAddress", "00:00:00:00:00:00"));
                    }
                }
                public void onTick(long timeRemaining){
                    if(reconnecting) {
                        progress.setMessage(progMsgBase +  "\n" + ctx.getString(R.string.Next_attempt_in) + " " + (int)(timeRemaining / 1000) + " " + ctx.getString(R.string.Seconds) + "...");
                    }else{
                        this.cancel();
                    }
                }
    ...
    private class ConnectThread extends Thread {
        ...
        public ConnectThread(BluetoothDevice device) {
            ...
            try{
                tmpSocket = btDevice.createRfcommSocketToServiceRecord(Constants.BASIC_UUID);
            }catch(Exception e){
                //device is null
            }
        ...
        public void run() {
            ...
            prefsEditor.putString("lastSuccessfulAddress", btDevice.getAddress());
            prefsEditor.commit();
