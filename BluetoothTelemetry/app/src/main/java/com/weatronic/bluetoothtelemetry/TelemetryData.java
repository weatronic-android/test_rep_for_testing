...
abstract class TelemetryData{
     ...
     * @param inValue unsigned value to be converted
     * @param sizeSymbols Based on type, e.g. 4 symbols = 2 bytes for Signed Short
     * @return Signed number
     */
    public static long unsignedToSigned(long inValue, int sizeSymbols){
        String maxPositiveStr = "7F";
        long outValue;
        for(int i = 0; i < sizeSymbols - 2; i++){
            maxPositiveStr += "F";
        }
        long maxPositive = Long.parseLong(maxPositiveStr, 16);
        if(inValue > maxPositive){
            outValue = 0 - (inValue - maxPositive);
        }else{
            outValue = inValue;
        }
        //Log.i("", "(unsigned_"+(sizeSymbols * 4)+"bit)"+inValue + " = (signed)"+outValue);
        return outValue;
    }
    /**
    ...
    /**
     * Returns size in hex symbols (1 byte = 2 symbols) for each data type. Sizes are fixed.
     * Number of symbols instead of just bytes is used because there is a 12bit type.
     * Another way would be to use floats as bytesizes.
     * @param type ID of datatype
     * @return Size in hex symbols
     */
...
class TelemetryDataNumber extends TelemetryDataSimple{
    ...
    /**
     * A shorter constructor for when neither M nor A are needed and the value comes ready for output.
     */
    ...
    /**
     * A shorter constructor for when an A-factor is not needed, which is very often.
     */
...
class TelemetryDataBitmask extends TelemetryDataNumber{
    ...
    public TelemetryDataBitmask(long ID, String name){
        //size in symbols * 4 = number of bits
        //2 ^ number of bits - 1 = maximum value
        super(ID, name, 0, Math.pow(2, sizeByType(idToType(ID)) * 4) - 1, "");
        this.sizeBits = sizeByType(idToType(ID)) * 4;
    }
...
abstract class TelemetryDataComposite extends TelemetryData{
     ...
     */
    abstract protected long newSubFieldID(long ID, int type);
    /**
    ...
    public void setValues(String inputString){
        ...
        for(int i = 0; i < subfields.size(); i++){
            try {
                ...
                pieceSize = sizeByType(field.type);
                //make sure it's a full byte
                if(pieceSize % 2 == 1) pieceSize++;
                endPos = curPos + pieceSize;
     ...
     * A tradeoff to keep the number of parameters for {@link #newSubFieldID(long, int)} minimal.
     */
 ...
 * It is therefore possible to build the IDs of subfields by keeping the ID and changing Index and Type. See {@link TelemetryDataCompositeDV4#newSubFieldID(long, int)}
 * Each of subfields is usually a {@link TelemetryDataSimple simple field}.
 */
class TelemetryDataCompositeDV4 extends TelemetryDataComposite{
     ...
     */
    @Override
    protected long newSubFieldID(long ID, int type){
...
class TelemetryDataGPS_DV4 extends TelemetryDataCompositeDV4{
    ...
    public TelemetryDataGPS_DV4(int ID, String name, Context ctx){
        super(ID, name);
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, SIGNED_WORD), this.name + " " + ctx.getResources().getString(R.string.Latitude), -90, 90, "°", (1.0f / 6000000.0f)));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, SIGNED_WORD), this.name + " " + ctx.getResources().getString(R.string.Longitude), -180, 180, "°", (1.0f / 6000000.0f)));
        subfields.add(new TelemetryDataNumberValidFlag(newSubFieldID(ID, UNSIGNED_SHORT), this.name + " " + ctx.getResources().getString(R.string.Speed), 0, 0, 60, "kn", 0.1f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, SIGNED_SHORT), this.name + " " + ctx.getResources().getString(R.string.Altitude), -1000, 8000, "m", 0.1f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), this.name + " " + ctx.getResources().getString(R.string.Course), 0, 360, "°", 0.01f));
        subfields.add(new TelemetryDataMsUTC(newSubFieldID(ID, UNSIGNED_WORD), this.name + " " + ctx.getResources().getString(R.string.UTC)));
...
class TelemetryDataArrayDV4 extends TelemetryDataCompositeDV4{
    ...
    public TelemetryDataArrayDV4(long ID, String name, String elementName, int size, int type, int firstIndex, Number limitMin, Number limitMax, String units, float factorK, int factorA){
        ...
        for(int i = 0; i < size; i++){
            //make sub fields
            TelemetryDataNumber newSubField = new TelemetryDataNumber(newSubFieldID(ID, type), elementName + " " + (firstIndex + i), limitMin, limitMax, units, factorK, factorA);
            subfields.add(newSubField);
            newSubField.type = type;
        }
        updateParentForSubfields();
    }
...
class TelemetryDataCompositeSkyNavReferential extends TelemetryDataComposite{
    ...
    protected long newSubFieldID(long ID, int type){
        ...
        if(type == SIGNED_12BIT)
            ...
            subType = type;
        String typeStr = Long.toBinaryString(subType);
        while(typeStr.length() < 5){
            typeStr = "0" + typeStr;
        }
        String finalIdStr = typeStr + baseID;
     ...
     * @return ID of newly created field
     */
    protected long newSubFieldID(long ID){
...
class TelemetryDataGPS_SkyNav extends TelemetryDataCompositeSkyNavReferential{
    ...
    public TelemetryDataGPS_SkyNav(int ID, String name, Context ctx){
        super(ID, name);
        String midBytesStr = Long.toHexString(ID).substring(1, 3);
        subfields.add(new TelemetryDataNumber(newSubFieldID(Long.parseLong(midBytesStr + "1", 16), SIGNED_WORD), this.name + " " + ctx.getResources().getString(R.string.Latitude), -90, 90, "°", (1.0f / 6000000.0f)));
        subfields.add(new TelemetryDataNumber(newSubFieldID(Long.parseLong(midBytesStr + "2", 16), SIGNED_WORD), this.name + " " + ctx.getResources().getString(R.string.Longitude), -180, 180, "°", (1.0f / 6000000.0f)));
        subfields.add(new TelemetryDataNumber(newSubFieldID(Long.parseLong(midBytesStr + "3", 16), UNSIGNED_SHORT), this.name + " " + ctx.getResources().getString(R.string.Speed), 0, 60, "kn", 0.1f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(Long.parseLong(midBytesStr + "4", 16), SIGNED_SHORT), this.name + " " + ctx.getResources().getString(R.string.Altitude), -1000, 8000, "m", 0.1f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(Long.parseLong(midBytesStr + "5", 16), UNSIGNED_SHORT), this.name + " " + ctx.getResources().getString(R.string.Course), 0, 360, "°", 0.01f));
        subfields.add(new TelemetryDataMsUTC(newSubFieldID(Long.parseLong(midBytesStr + "8", 16), UNSIGNED_WORD), this.name + " " + ctx.getResources().getString(R.string.UTC)));
        subfields.add(new TelemetryDataNumber(newSubFieldID(Long.parseLong(midBytesStr + "9", 16), UNSIGNED_BYTE), this.name + " " + ctx.getResources().getString(R.string.FracUTC), 0, 996, "ms", 4.0f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(Long.parseLong(midBytesStr + "A", 16), SIGNED_SHORT), this.name + " " + ctx.getResources().getString(R.string.AltitudeRelative), -1000, 8000, "m", 0.1f));
        updateParentForSubfields();
    }
}
/**
...
class TelemetryDataArraySkyNav extends TelemetryDataCompositeSkyNavReferential{
    ...
    public TelemetryDataArraySkyNav(long ID, String name, String elementName, int size, int type, int firstIndex, long firstID, Number limitMin, Number limitMax, String units, float factorK, int factorA){
        ...
        for(int i = 0; i < size; i++){
            //make sub fields
            TelemetryDataNumber newSubField = new TelemetryDataNumber(newSubFieldID(firstID + i), elementName + " " + (firstIndex + i), limitMin, limitMax, units, factorK, factorA);
            subfields.add(newSubField);
            newSubField.type = type;
        }
        updateParentForSubfields();
 ...
 * See {@link @TelemetryDataCompositeSkyNavSolid#newSubFieldID} for how and ID is built.
...
class TelemetryDataCompositeSkyNavSolid extends TelemetryDataComposite{
     ...
     * Therefore, a new ID is built by adding a 2-byte "index" at the end (see {@link TelemetryDataCompositeDV4#newSubFieldID(long, int)}.
    ...
    protected long newSubFieldID(long ID, int type){
        String baseID = Long.toBinaryString(ID);
        while(baseID.length() < 16){
            baseID = "0" + baseID;
        }
        baseID = baseID.substring(5, baseID.length());
        //virtual type, not used in IDs
        int subType;
        if(type == SIGNED_12BIT)
            subType = SIGNED_SHORT;
            ...
            subType = type;
        String typeStr = Integer.toBinaryString(subType);
        while(typeStr.length() < 16){
            typeStr = "0" + typeStr;
        }
        ...
        String finalIdStr = typeStr + baseID + indexStr;
        long finalId = Long.parseLong(finalIdStr, 2);
        this.children.add(finalId);
        return finalId;
    }
...
class TelemetryDataPowerSupply extends TelemetryDataCompositeSkyNavSolid{
    ...
    public TelemetryDataPowerSupply(int ID, String name, Context ctx){
        ...
        subfields.add(new TelemetryDataBitmask(newSubFieldID(ID, UNSIGNED_WORD), PS + " " + ctx.getResources().getString(R.string.Status)));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " +  ctx.getResources().getString(R.string.Voltage), 0, 20, "V", 0.001f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " +  ctx.getResources().getString(R.string.Current), -20, 20, "A", 0.001f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " +  ctx.getResources().getString(R.string.Input_voltage), 0, 20, "V", 0.001f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " +  ctx.getResources().getString(R.string.Input_current), -20, 20, "A", 0.001f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " +  ctx.getResources().getString(R.string.Main_voltage), 0, 20, "V", 0.001f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " +  ctx.getResources().getString(R.string.Reserve_voltage), 0, 20, "V", 0.001f));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, SIGNED_BYTE), PS + " " +  ctx.getResources().getString(R.string.Input_temperature), -40, 40, "°С"));
        subfields.add(new TelemetryDataNumber(newSubFieldID(ID, SIGNED_12BIT), PS + " " +  ctx.getResources().getString(R.string.Reserved), 0, 0, ""));
        for(int cell = 1; cell <= 4; cell++){
            subfields.add(new TelemetryDataBitmask(newSubFieldID(ID, UNSIGNED_WORD), PS + " " + cellStr + cell + " " + ctx.getResources().getString(R.string.Status)));
            subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " + cellStr + cell + " " + ctx.getResources().getString(R.string.Voltage), 0, 20, "V", 0.001f));
            subfields.add(new TelemetryDataNumber(newSubFieldID(ID, SIGNED_SHORT), PS + " " + cellStr + cell + " " + ctx.getResources().getString(R.string.Current), -20, 20, "A", 0.001f));
            subfields.add(new TelemetryDataNumber(newSubFieldID(ID, UNSIGNED_SHORT), PS + " " + cellStr + cell + " " + ctx.getResources().getString(R.string.Capacity), 0, 20, "mAh"));
            subfields.add(new TelemetryDataNumber(newSubFieldID(ID, SIGNED_BYTE), PS + " " + cellStr + cell + " " + ctx.getResources().getString(R.string.Temperature), -40, 40, "°С"));
        }
        updateParentForSubfields();
    }
}
...
class TelemetryDataArrayVirtual extends TelemetryDataComposite{
    ...
    public TelemetryDataArrayVirtual(long startID, long diffID, String name, String elementName, int size, Number limitMin, Number limitMax, String units, float factorK, int factorA){
        ...
        //no real field (or field generated by TelemetryDataCompositeSkyNavSolid) should overlap
        ...
        for(int i = 0; i < size; i++){
            ...
            subfields.add(new TelemetryDataNumber(newSubFieldID(nextID, 0), elementName + " " + (i + 1), limitMin, limitMax, units, factorK, factorA));
        }
        updateParentForSubfields();
    }
